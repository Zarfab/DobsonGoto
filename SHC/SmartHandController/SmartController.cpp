...
void SmartHandController::setup(const char version[], const int pin[7],const bool active[7], const OLED model)
{
  // get "EEPROM" ready
  nv.init();
  ...
  // Chose and initialize communication
  // select SERIAL_INTERFACE if ST4 De+ and De- are both pulled HIGH, 
  ...
  if(digitalRead(ST4DEs) && digitalRead(ST4DEn)) {
      ...
      SERIAL_INTERFACE_ALT.begin(SERIAL_BAUD_DEFAULT);
      Ser = &SERIAL_INTERFACE_ALT;
      alternativeSerial = true;
  }
  // establish comms and clear the channel
  //Ser->begin(SerialBaud); // already begun
  ...
  DisplayMessage(L_CONNECTION " over", alternativeSerial? SERIAL_INTERFACE_ALT_NAME : SERIAL_INTERFACE_NAME, 2500);
  DisplayMessage(L_ESTABLISHING, L_CONNECTION, 100);
...
#else
    if (!moveEast  && (buttonPad.e.isDown())) { 
        moveEast = true;   
        if(!alternativeSerial) Ser->write(ccMe); else SetLX200(":Me#");
        buttonCommand=true; } 
    else if (moveEast   && (buttonPad.e.isUp())) { 
        moveEast = false;  
        if(!alternativeSerial) Ser->write(ccQe); else SetLX200(":Qe#");
        buttonCommand=true; 
        buttonPad.e.clearPress(); 
    }
    if (!moveWest  && (buttonPad.w.isDown())) { 
        moveWest = true;   
        if(!alternativeSerial) Ser->write(ccMw); else SetLX200(":Mw#"); 
        buttonCommand=true; } 
    else if (moveWest   && (buttonPad.w.isUp()  )) { 
        moveWest = false;  
        if(!alternativeSerial) Ser->write(ccQw); else SetLX200(":Qw#"); 
        buttonCommand=true; 
        buttonPad.w.clearPress(); 
    }
    if (!moveNorth && (buttonPad.n.isDown())) { 
        moveNorth = true;  
        if(!alternativeSerial) Ser->write(ccMn); else SetLX200(":Mn#"); 
        buttonCommand=true; 
    } 
    else if (moveNorth  && (buttonPad.n.isUp())) { 
        moveNorth = false; 
        if(!alternativeSerial) Ser->write(ccQn); else SetLX200(":Qn#"); 
        buttonCommand=true; 
        buttonPad.n.clearPress(); 
    }
    if (!moveSouth && (buttonPad.s.isDown())) { 
        moveSouth = true;  
        if(!alternativeSerial) Ser->write(ccMs); else SetLX200(":Ms#");  
        buttonCommand=true; 
    } 
    else if (moveSouth  && (buttonPad.s.isUp())) { 
        moveSouth = false; 
        if(!alternativeSerial) Ser->write(ccQs); else SetLX200(":Qs#"); 
        buttonCommand=true; 
        buttonPad.s.clearPress(); 
    }
    //if(alternativeSerial) Ser->flush();
#endif
